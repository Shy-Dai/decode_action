...
class MainProgram {
    ...
    async login(){
        ...
        let body = JSON.stringify({"field":this.userName,"password":this.password});
        let headers = this.headers;
        let response = await post({url,body,headers});
    ...
    async icon_login(){
        ...
        let url = 'https://api.v2.rainyun.com/user/login'
        let body = JSON.stringify({"field":this.userName,"password":this.password,"vticket":r.ticket,"vrandstr":r.randstr});
        let headers = this.headers;
        let response = await post({url,body,headers});
        let cookies = response.headers['set-cookie'];
        response = JSON.parse(response.body);
    ...
    async signIn(){
        ...
        let body = JSON.stringify({"task_name":"每日签到","verifyCode":"","vticket":r.ticket,"vrandstr":r.randstr});
        let headers = this.headers;
        let response = JSON.parse((await post({url,body,headers})).body);
        if(response.code === 200){
            this.log("签到成功")
        }else{
            this.log(response.message);
    ...
    async sign_info(){
        let url = 'https://api.v2.rainyun.com/user/reward/tasks'
        let headers = this.headers;
        let response = JSON.parse((await get({url,headers})).body);
